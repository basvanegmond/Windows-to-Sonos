...
| Task runs but nothing listens, `logs\server.log` shows "Unable to configure formatter 'default'" | An old `app.py` without `_ensure_streams()`; pythonw gives the process no stdout and uvicorn crashes on it. Pull the current version |
